...
class Cp_zendesk_ext
{
	...
	function add_js()
	{
		...
		if ($this->settings['zendesk_dropbox_version'] == 'version_1') {
			...
			$replace .= '            tabImageURL:         "'.'http'.(isset($_SERVER["HTTPS"]) ? 's' : '').'://'.$_SERVER["SERVER_NAME"].constant('PATH_CP_GBL_IMG').'zendesk_tabs/'.$this->settings['zendesk_param_tab_position'].'/tab_'.$this->settings['zendesk_param_tab_id'].'_front.png'.'",'."\n";
			...
			$replace .= '            requester_name:      "'.$this->EE->session->userdata['screen_name'].'",'."\n";
			$replace .= '            requester_email:     "'.$this->EE->session->userdata['email'].'"'."\n";
			$replace .= '         });'."\n";
			$replace .= '      }'."\n";
			$replace .= '   });'."\n";
			$replace .= '</script>'."\n";
		...
		$replace .= '      $("#zendesk_param_title").parent().parent().before("<tr><th colspan=\"2\">Version 1 specific inputs:</th></tr>");'."\n";
		$replace .= '      $("#zendesk_param_drobox_id").parent().parent().before("<tr><th colspan=\"2\">Version 2 specific inputs:</th></tr>");'."\n";
		$replace .= '      $("#zendesk_dropbox_version_version_1").parent().parent().parent().parent().before("<div style=\"margin-left: 10px;\">** &nbsp;=&nbsp; Values which must be entered for the ZenDesk dropbox to load</div>");'."\n\n";
		if ($version1)
			$replace .= '      Zenbox.init();'."\n";
